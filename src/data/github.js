...
function orgOrUserFromString(url) {
 ...
 */
export async function sniffForGithubOrgOrUser(workspaceRootURI) {
    // package.json repository
    try {
        const pkg = JSON.parse(Buffer.from(await workspace.fs.readFile(vscode.Uri.joinPath(workspaceRootURI, 'package.json'))).toString());
    ...
    // git remotes?
    try {
        const gitConfig = ini.parse(Buffer.from(await workspace.fs.readFile(vscode.Uri.joinPath(workspaceRootURI, '.git', 'config'))).toString());
...
export function installGithubApp(uri) {
    vscode.authentication.getSession('github', [
        ...
        if (id) {
            vscode.env.openExternal(vscode.Uri.parse(`https://github.com/apps/socket-security/installations/new/permissions?target_id=${id}`));
        }
        else {
            vscode.env.openExternal(vscode.Uri.parse(`https://github.com/apps/socket-security/installations/new/`));
        }
    }, () => {

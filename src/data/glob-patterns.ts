    ...
    globPatternsPromise = (once(req, 'response') as Promise<[IncomingMessage]>)
      ...
      .catch(err => {
        ...
        return {
          ...
          golang: {
            gomod: { pattern: 'go.mod' },
            gosum: { pattern: 'go.sum' },
          },
          maven: {
            buildr: { pattern: 'Buildfile' },
            gradle: { pattern: '{*.,}gradle{.lockfile,}' },
            ivy: { pattern: 'ivy.xml' },
            kotlin: { pattern: '*.gradle.kts' },
            leiningen: { pattern: 'project.clj' },
            pomxml: { pattern: '{*-*.,}pom{.xml,}' },
            sbt: { pattern: 'build.sbt' },
          },
          npm: {
            npmshrinkwrap: { pattern: 'npm-shrinkwrap.json' },
            packagejson: { pattern: 'package.json' },
            packagelockjson: { pattern: 'package-lock.json' },
            pnpmlock: { pattern: 'pnpm-lock.y{a,}ml' },
            pnpmworkspace: { pattern: 'pnpm-workspace.y{a,}ml' },
            yarnlock: { pattern: 'yarn.lock' },
          },
          pypi: {
            'cdx-json': {
              pattern: '{bom,c{yclone,}dx[-.]*,*[-.]c{yclone,}dx}.json',
            },
            'cdx-xml': {
              pattern: '{bom,c{yclone,}dx[-.]*,*[-.]c{yclone,}dx}.xml',
            },
            pipfile: { pattern: 'pipfile' },
            piplock: { pattern: 'pipfile.lock' },
            pkginfo: { pattern: '{PKG-INFO,METADATA}' },
            poetry: { pattern: 'poetry.lock' },
            pylock: { pattern: 'pylock{,.*}.toml' },
            ...
            'spdx-json': { pattern: '*[-.]spdx.json' },
            uvlock: { pattern: 'uv.lock' },
          },
          spdx: { json: { pattern: '*[-.]spdx.json' } },
          nuget: {
            visualStudioSolution: { pattern: '*.sln' },
            msbuildProject: { pattern: '*.*proj' },
            targets: { pattern: '*.targets' },
            props: { pattern: '*.props' },
            msbuildProjectItems: { pattern: '*.projitems' },
            nuspec: { pattern: '*.nuspec' },
            packageConfig: { pattern: '{packages.*.config,packages.config}' },

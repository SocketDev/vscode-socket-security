...
import * as auth from './installation-settings/auth';
'./auth';
export async function activate(context) {
    await auth.activate(context);
    context.subscriptions.push(vscode.commands.registerCommand(`${EXTENSION_PREFIX}.installGitHubApp`, installGithubApp), vscode.commands.registerCommand(`${EXTENSION_PREFIX}.ignoreIssueType`, async (from, type) => {
    ...
    context.subscriptions.push(diagnostics, watchers['package-lock.json'].watch(watchHandler), watchers['package.json'].watch(watchHandler));
    if (workspace.workspaceFolders) {
        for (const workFolder of workspace.workspaceFolders) {
            populateDiagnostics(workFolder.uri);
        }
    }
    context.subscriptions.push(reports.onReport(null, async (evt) => {
    ...
    function normalizeReportAndLocations(report, doc) {
        const externals = parseExternals(doc);
        if (!externals) {
            return;
        }
        const issuesForSource = radixMergeReportIssues(report);
        let ranges = Object.create(null);
        let prioritizedRanges = Object.create(null);
        function pushRelatedRange(name, range) {
    ...
    async function populateDiagnostics(workspaceFolderURI) {
        ...
        for (const textDocumentURI of workspacePkgs) {
            ...
            if (relevantIssues && relevantIssues.length) {
                const diagnosticsToShow = (await Promise.all(relevantIssues.map(async (issue) => {
                    const should = shouldShowIssue(issue.type, issue.severity, socketYamlConfig);
                    if (!should) {
                        return null;
                    }
                    const diag = new vscode.Diagnostic(issue.range, issue.description, issue.severity === 'low' ?
                        vscode.DiagnosticSeverity.Information :
                        issue.severity !== 'critical' ?
                            vscode.DiagnosticSeverity.Warning :
                            vscode.DiagnosticSeverity.Error);
                    diag.relatedInformation = issue.related.map((r) => new vscode.DiagnosticRelatedInformation(new vscode.Location(textDocumentURI, r), 'installation reference'));

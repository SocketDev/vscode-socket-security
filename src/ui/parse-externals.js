...
function getPackageNameFromSpecifier(name) {
    return (name.startsWith('@') ?
        name.split('/', 2) :
        name.split('/', 1)).join('/');
}
function getPackageNameFromVersionRange(name) {
    return (name.startsWith('@') ?
        name.split('@', 3) :
        name.split('@', 2)).join('@');
}
export function parseExternals(doc) {
    ...
    if (SUPPORTED_LANGUAGE_IDS.includes(doc.languageId)) {
        ...
        function addResult(node, specifier) {
            if (/^[\.\/]/u.test(specifier)) {
                return;
            }
        ...
        const kDYNAMIC_VALUE = Symbol('dynamic_value');
        /**
...
function rangeForJSONAstLoc(loc) {
    return new vscode.Range(new vscode.Position(loc.start.line - 1, loc.start.column - 1), new vscode.Position(loc.end.line - 1, loc.end.column - 1));
}
function parsePkgOverrideExternals(node, results, contextualName) {
...
//# sourceMappingURL=parse-externals.js.map
...
Drives the Cursor Bugbot fix-and-respond loop end-to-end. This is the canonical flow every PR author should run after Bugbot posts findings.

## Modes

- `/driving-cursor-bugbot <PR#>` — full audit-and-fix on one PR (default).
- `/driving-cursor-bugbot check <PR#>` — list Bugbot findings, classify them, but don't fix or reply.
- `/driving-cursor-bugbot reply <comment-id> <state>` — single-comment reply where `<state>` is `fixed`, `false-positive`, or `wont-fix`. Auto-resolves the thread for `fixed` / `false-positive`; leaves open for `wont-fix`.
- `/driving-cursor-bugbot resolve <PR#>` — sweep all Bugbot threads on the PR that have author replies but are still open; resolve them. Useful for closing out a PR before merge.
- `/driving-cursor-bugbot scope <PR#>` — re-evaluate the PR title and body against the actual commits and rewrite them when out of step.
...
Each Bugbot finding is its own logical fix. Don't bundle.

- **One commit per `real` finding.** A typo fix and a memory-leak fix that happen on the same Bugbot pass are still two separate concerns. Reviewers diff per finding.
- **Push after each fix; reply with the new commit SHA.** The reply on the inline thread cites the SHA (`Fixed in <sha>`), so the SHA must already be pushed when you reply.
- **Don't stack uncommitted fixes across phases.** If you stop the skill mid-flow (interrupted, blocked on a question), commit what's done — partial-feature dirty branches are exactly what the umbrella rule forbids. Resume on the next session by re-running the skill.
- Conventional commit shape: `fix(<scope>): address Bugbot finding on <file>:<line>`.
